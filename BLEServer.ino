...
void setup() {
   ...
   //pinMode(LED, OUTPUT);
    ...
    Serial.println("Starting BLEServer");
...
//??original    NimBLEDevice::init("NimBLE-Arduino");
 ...
 //       NIMBLE_PROPERTY::NOTIFY
    ...
    pFoodCharacteristic->setValue("PT pFoodChar sent");
    ...
    pPetTutordsc->setCallbacks(&dscCallbacks);
...
void loop() {
    ...
    if (pServer->getConnectedCount() == 0) {  //?? flash led if no clients connected. note: advertising continues with clients connected  wha 8-26-21
       // BlinkLed();
    }
    else {
    ...
    delay(2000);  //this delays the feed trigger response
}
